...
import { RefreshCw, Settings, Link as LinkIcon, AlertCircle, ShieldCheck } from 'lucide-react'
import { Card, CardContent, Button, Label, Select } from '../components/ui'
...
function generateMock(limit=50): Row[] {
  const base:[string,string,string][]= [['RELIANCE','Reliance Industries','Oil & Gas'],['TCS','Tata Consultancy Services','IT & Software'],['HDFCBANK','HDFC Bank','Banks'],['ICICIBANK','ICICI Bank','Banks'],['LT','Larsen & Toubro','Capital Goods'],['ITC','ITC','FMCG'],['SBIN','State Bank of India','Banks'],['MARUTI','Maruti Suzuki','Auto & Auto Ancillaries'],['SUNPHARMA','Sun Pharma','Pharma & Healthcare'],['ULTRACEMCO','UltraTech Cement','Cement & Materials'],['BHARTIARTL','Bharti Airtel','Telecom'],['NTPC','NTPC','Power & Utilities'],['POWERGRID','Power Grid','Power & Utilities'],['INFY','Infosys','IT & Software'],['HCLTECH','HCLTech','IT & Software'],]
  const rows:Row[]=[]; for(let i=0;i<limit;i++){ const [sym,nm,ind]=base[i%base.length]; const basePrice=Math.random()*2000+100; const ch=(Math.random()*6-1); const last=parseFloat((basePrice*(1+ch/100)).toFixed(2)); const prev=parseFloat(basePrice.toFixed(2)); const volume=Math.floor(Math.random()*5_000_000+50_000); rows.push({ symbol:sym, name:nm, industry:ind, last, prev, change_pct:parseFloat(ch.toFixed(2)), volume, updated_at:new Date().toISOString() }) } rows.sort((a,b)=>b.change_pct-a.change_pct); return rows.slice(0,limit)
}
...
export default function Page(){
  ...
  const [loading,setLoading]=useState(false); const [error,setError]=useState(''); const [rows,setRows]=useState<Row[]>([]); const [connected,setConnected]=useState<boolean>(false)

  const industries=useMemo(()=>{ const fromData=Array.from(new Set(rows.map(r=>r.industry).filter(Boolean))) as string[]; return ['All', ...new Set([...DEFAULT_INDUSTRIES.filter(i=>i!=='All'), ...fromData])] },[rows])
  useEffect(()=>{localStorage.setItem('nseUseMock',String(useMock))},[useMock])

  async function checkStatus(){ try{ const res=await fetch('/api/kite/status',{cache:'no-store'}); const j=await res.json(); setConnected(Boolean(j?.ok)) }catch{ setConnected(false) } }

  async function fetchData(){ setLoading(true); setError(''); try{ let data:Row[]=[]; if(useMock){ data=generateMock(limit) } else { const url=new URL('/api/top-gainers', window.location.origin); url.searchParams.set('limit', String(limit)); if(industry!=='All') url.searchParams.set('industry', industry); const res=await fetch(url.toString(), { cache:'no-store' }); if(!res.ok) throw new Error(`API ${res.status}`); const j=await res.json(); data=Array.isArray(j?.data)?j.data:[] } let next=data.filter(r=>typeof r.change_pct==='number').sort((a,b)=>b.change_pct-a.change_pct); if(industry!=='All') next=next.filter(r=>(r.industry||'').toLowerCase()===industry.toLowerCase()); setRows(next.slice(0,limit)) } catch(e:any){ setError(e?.message||'Failed to load data'); setRows([]) } finally { setLoading(false) } }

  useEffect(()=>{ checkStatus(); fetchData() },[industry,limit,useMock])
  useEffect(()=>{ if(!refreshSec) return; const id=setInterval(()=>{ checkStatus(); fetchData() }, refreshSec*1000); return ()=>clearInterval(id) },[refreshSec,useMock,industry,limit])
  ...
  return (<div className='min-h-screen p-4 md:p-8 max-w-6xl mx-auto'>
    <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:0.35}} className='space-y-4'>
      <div className='flex flex-col gap-4 md:flex-row md:items-end md:justify-between'>
        <div><h1 className='text-2xl font-bold'>NSE Top Gainers (Last 1 Hour)</h1><p className='text-sm text-gray-500'>Top 50 across all industries • Auto-refresh {refreshSec}s • Window: 60 minutes</p>
          <div className='flex items-center gap-2 mt-2'><ShieldCheck className={`w-4 h-4 ${connected?'text-green-600':'text-gray-400'}`} /><span className={`text-xs ${connected?'text-green-700':'text-gray-500'}`}>{connected?'Zerodha: Connected':'Zerodha: Not connected'}</span>{!connected && <Button href='/api/kite/login' className='ml-3'>Login with Zerodha</Button>}</div>
        </div>
      ...
      <Card><CardContent><div className='flex items-center gap-2 mb-2'><Settings className='w-5 h-5'/><h2 className='font-semibold'>Data Source</h2></div>
        <div className='grid gap-3 md:grid-cols-3'>
          <div className='flex flex-col'><Label>Mode</Label><Select value={useMock?'mock':'live'} onChange={(v)=>setUseMock(v==='mock')}><option value='mock'>Demo (mock data)</option><option value='live'>Live (use API)</option></Select><p className='text-xs text-gray-500 mt-1'>Use demo mode now; switch to Live after Zerodha login.</p></div>
          <div className='flex flex-col md:col-span-2'><Label>How it works</Label><p className='text-sm text-gray-600 flex items-center gap-1'><LinkIcon className='w-4 h-4'/> Frontend calls <code>/api/top-gainers</code>, which uses your Zerodha token set by <code>/api/kite/callback</code>.</p></div>
        </div></CardContent></Card>
      <div className='text-xs text-gray-500'>Built with Next.js + Tailwind. Click “Login with Zerodha” to connect and switch Mode to Live.</div>
    </motion.div></div>)
}
