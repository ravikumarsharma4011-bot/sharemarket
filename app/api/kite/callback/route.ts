...
export async function GET(req: NextRequest) {
  ...
  if (!request_token) {
    return NextResponse.json({ ok: false, error: 'Missing request_token' }, { status: 400 })
  }
  const api_key = process.env.KITE_API_KEY || ''
  const api_secret = process.env.KITE_API_SECRET || ''
  if (!api_key || !api_secret) {
    return NextResponse.json({ ok: false, error: 'Server missing API credentials' }, { status: 500 })
  }
  ...
  if (!res.ok || !json?.data?.access_token) {
    return NextResponse.json({ ok: false, error: json?.message || 'Token exchange failed' }, { status: 400 })
  }

  const accessToken = json.data.access_token as string
  // Token is valid for the trading day; store in secure cookie (~20 hours)
