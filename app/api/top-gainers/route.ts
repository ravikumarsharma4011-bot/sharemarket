...
function generateMock(limit=50): Row[] {
  ...
  const rows:Row[]=[]; for(let i=0;i<limit;i++){ const [sym,nm,ind]=base[i%base.length]; const basePrice=Math.random()*2000+100; const ch=(Math.random()*6-1)
    const last=parseFloat((basePrice*(1+ch/100)).toFixed(2)); const prev=parseFloat(basePrice.toFixed(2)); const volume=Math.floor(Math.random()*5_000_000+50_000)
  ...
  rows.sort((a,b)=>b.change_pct-a.change_pct); return rows.slice(0,limit)
}
